...
#host = '10.0.0.241' #ip of raspberry pi
#host = '10.0.0.61'
#readPort = 2800

reader = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
server_address = ('10.0.0.61', 3800)
#reader.connect(( '10.0.0.61', 3800))
#reader.send('hello')

#reader.bind((host, readPort))
#reader.listen(1)
#reader, addr1 = reader.accept()
#print ('Got connection from',addr1)
#reader.send('Connected\n')
...
from math import atan, atan2, sqrt
import smbus
commBus = smbus.SMBus(1);
class PyComms:
    def __init__(self, address, bus = commBus ):
...
def read_byte(adr):
    return commBus.read_byte_data(address, adr)
def read_word(adr):
    high = commBus.read_byte_data(address, adr)
    low = commBus.read_byte_data(address, adr+1)
    val = (high << 8) + low
    return val
def read_word_2c(adr):
    ...
    if (val >= 0x8000):
        ...
        return val
def calculateYPRACCL(name)  :
        global robotYaw,accel_xout, accel_yout
        ...
        if mpuIntStatus >= 2: # check for DMP data ready interrupt (this should happen frequently)
            ...
            #time.sleep(0.01);
def wifiSocketCommunicate(name) :
	while 1:
		data,server = reader.recvfrom(1024)
		if data:
			arduinoSerial.write(data)
			print "data ", data
		#print "received data:", parseString(data)[0], parseString(data)[1] #TODO replace with serial.write
		#arduinoSerial.write(str(parseString(data)[0]) + ";" + str(parseString(data)[1]) + ";\n");
		s = str(robotYaw) + ";" + str(accel_xout) + ";" + str(accel_yout) + ";" + str(float(rightMotorCount)) + ";" + str(float(leftMotorCount)) + ";\n"  #TODO choose wat to send
		reader.send(s)
		sleep(0.01)

#start_new_thread(calculateYPRACCL,("Thread1",))
#start_new_thread(wifiSocketCommunicate("Thread2",))
while 1:
	data = reader.recvfrom(24)
	if data:
		arduinoSerial.write(data)
		print "data ", data
	#print "received data:", parseString(data)[0], parseString(data)[1] #TODO replace with serial.write
...
#calculateYPRACCL("name")
#print robotYaw,accel_xout, accel_yout
#while True:
#    calculateYPRACCL("name")
#    print robotYaw,accel_xout, accel_yout
