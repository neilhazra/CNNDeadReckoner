...
void setup()
{
  ...
  Serial.begin(9600);  // start serial for output
	...
	Wire.write(INTEG_MODE_FREE | INTEG_PARAM_PULSE_COUNT1);
	Wire.endTransmission();  
	delay(10); 

        Wire.beginTransmission(sensorAddress_);
	Wire.write(REG_INT_SOURCE);
	Wire.write(INT_SOURCE_CLEAR);
	Wire.endTransmission();  
	delay(10);
...
void loop()
{
 ...
 if(8 <= Wire.available())
	...
	int green_	= readingdata_[1] * 256 + readingdata_[0];
	int red_ 	= readingdata_[3] * 256 + readingdata_[2];
	int blue_	= readingdata_[5] * 256 + readingdata_[4];
	int clear_	= readingdata_[7] * 256 + readingdata_[6];
