...
class AccelerometerBind():
    def __init__(self):
        ...
        self.F = open("/home/pi/MLRobot/Data/AccelRawDyn" + time.strftime("%e:%H:%M:%S", time.localtime(time.time())),"w");
    ...
    def AccelerationChangeHandler(self,e, acceleration, angularRate , magneticField, timestamp):
        accel =  [0,0,0]
        gyro = [0,0,0]
        accel[0] = acceleration[0]#/sqrt(acceleration[0]**2+acceleration[1]**2 + acceleration[2]**2)
        accel[1] = acceleration[1]#/sqrt(acceleration[0]**2+acceleration[1]**2 + acceleration[2]**2)
        accel[2] = acceleration[2]#/sqrt(acceleration[0]**2+acceleration[1]**2 + acceleration[2]**2)
        ...
        if not self.stopped:
            ...
            if not LA.norm(gyro) < 0.005:
                ...
                self.q = self.q_mult(q_update,self.q) #TODO
            self.dynAccel = self.qv_mult(self.q_conjugate(self.q),accel) # conjugate first to get reverse rotation
            self.velocity = np.add(self.velocity, (deltaT/2)*np.add(self.dynAccel,self.previousDynAccel)*9.81)
            self.distance = np.add(self.distance, (deltaT/2)*np.add(self.velocity,self.previousVelocity))

            print self.distance[0], self.distance[1], self.velocity[0], self.velocity[1], self.dynAccel[0], self.dynAccel[1]
...
output = OutputData(0) #initial values0
...
class EncoderIntegration(threading.Thread):
    ...
    def __init__(self):
        ...
        self.running = False
        self.initialTime = time.time()
        self.F = open("/home/pi/MLRobot/Data/EncoderRawIntegrated" + time.strftime("%e:%H:%M:%S", time.localtime(time.time())),"w");
    def run(self):
        ...
        while True:
            ...
            if self.running:
                ...
                #print str(_in) + " " + str(time.time()*1000-self.initialTime*1000)
                d = _in.split("\n")[0].split(";")
                try:
...
while True:
    ...
    while time.time()*1000-initial<(inputData.mytime+500) or not abs(E.velocityy) < 0.001 : #wait half second extra
        dt = time.time()-prevtime
        prevtime = time.time()
        ...
        rotation_matrix = np.identity(2) #np.asarray([[cos(radians(AccelBind.heading)),sin(radians(AccelBind.heading))],
        ...
        R = np.asarray(np.diag((0.001,0.001,0.002,0.02))) #from encoder
        [x1,p1] = kalman(y,x_0,p_0,F_k,B_k, u_k,H_k,A,R)
        x_0 = x1
        p_0 = p1
        #print(AccelBind.dynAccel)
        sleep(0.01)
    E.stop()
    AccelBind.stop()
    output.update(voltage,E.x,E.y,0,0,AccelBind.distance[0],AccelBind.distance[1])
    AccelBind.reset()
    E.reset()
    s = str(output.Voltage)+";"+str(output.Ex)+";"+str(output.Ey)+";"+str(output.accelIntX)+";"+str(output.accelIntY) +";\n" #create data string
    dataSocket.send(s)
    rightMotorCount = 0
    leftMotorCount = 0
