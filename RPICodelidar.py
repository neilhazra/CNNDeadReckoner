...
#dataSocket.connect(( '10.0.0.139', 3801)) #Dad laptop
...
class AccelerometerBind():
    def __init__(self):
        ...
        self.F = open("/home/pi/MLRobot/Data2/AccelRawDyn" + time.strftime("%m-%e-%H-%M-%S", time.localtime(time.time())) + '.csv',"w");
    ...
    def AccelerationChangeHandler(self,e, acceleration, angularRate , magneticField, timestamp):
        ...
        if not self.stopped:
            ...
            self.dynAccel = [self.dynAccel[0],self.dynAccel[1],self.dynAccel[2]]
...
E.start()
AccelBind.ch.setDataInterval(int(500))
kalmFile = open("/home/pi/MLRobot/Data2/KalmanFilterRealTime-" + time.strftime("%m-%e-%H-%M-%S", time.localtime(time.time())) + '.csv',"w");
logger =  open("/home/pi/MLRobot/Data2/FullLogger-" + time.strftime("%m-%e-%H-%M-%S", time.localtime(time.time())) + '.csv',"w");
pidFile =  open("/home/pi/MLRobot/PID-" + time.strftime("%m-%e-%H-%M-%S", time.localtime(time.time())) + '.csv',"w");
line = 'trialNumber' + ',' + 'iteration' + ',' + 'time' + ',' + 'leftMotorCount' + ',' + 'AccelBind.distance[0]' + ',' + 'E.x' + ',' + 'leftMotorPower' + ',' + 'deltaLeftMotorCount' + ',' + 'deltaAccDispX' + ',' + 'deltaEncDispX' + ',' + 'rightMotorCount' + ',' + 'AccelBind.distance[1]' + ',' + 'E.y' + ',' + 'rightMotorPower' + ',' + 'deltaRightMotorCount' + ',' + 'deltaAccDispY' + ',' + 'deltaEncDispY'+ ',' + 'pidOut' + '\n'
pidFile.write(line);
#Main Trial Loop
while True:
    ...
    prevLeftMotorCount = prevRightMotorCount = 0
    ...
    while (time.time()-initTime)*1000 < (inputData.mytime):
        ...
        deltaAccDispX = AccelBind.distance[0] - prevAccDispX
        deltaAccDispY = AccelBind.distance[1] - prevAccDispY
        deltaLeftMotorCount = leftMotorCount - prevLeftMotorCount
        deltaRightMotorCount = rightMotorCount - prevRightMotorCount
        #store past cycle data
        prevEncDispX = E.x
        prevEncDispY = E.y
        prevAccDispX = AccelBind.distance[0]
        prevAccDispY = AccelBind.distance[1]
        prevLeftMotorCount = leftMotorCount
        prevRightMotorCount = rightMotorCount
        ...
        #print 'Accelerometer distance Delta:' + str(deltaAccDispX) + '/' + str(deltaAccDispY) + " Total: " + str(AccelBind.distance[0]) + '/' + str(AccelBind.distance[1])
        #print 'Encoder count Delta:' + str(deltaLeftMotorCount) + '/' + str(deltaRightMotorCount) + " Total: " + str(leftMotorCount) + '/' + str(rightMotorCount)
        #print 'Enoder distance Delta:' + str(deltaEncDispX) + '/' + str(deltaEncDispY) + " Total: " + str(E.x) + '/' + str(E.y)
        #print 'PID Out:' + str(pidOut)
        #print 'leftPower/rightPower:' + str(leftMotorPower) +  '/' + str(rightMotorPower)
        #print '-----------------------------------------------------------------------------'
        line = str(trialNumber) + ',' + str(iteration) + ',' + str(time.time()) + ',' + str(leftMotorCount) + ',' + str(AccelBind.distance[0]) + ',' + str(E.x) + ',' + str(leftMotorPower) + ',' + str(deltaLeftMotorCount) + ',' + str(deltaAccDispX) + ',' + str(deltaEncDispX) + ',' + str(rightMotorCount) + ',' + str(AccelBind.distance[1]) + ',' + str(E.y) + ',' + str(rightMotorPower) + ',' + str(deltaRightMotorCount) + ',' + str(deltaAccDispY) + ',' + str(deltaEncDispY) + ',' + str(pidOut) + '\n'
        pidFile.write(line);
