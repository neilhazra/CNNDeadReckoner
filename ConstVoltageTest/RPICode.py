...
while True:
    ...
    while time.time()*1000-initial<(inputData.mytime+1000) or not abs(E.velocityy) < 0.001 : #wait half second extra
        dt = time.time()-prevtime
        prevtime = time.time()
        ...
        u_k = np.asarray([raw[0],               #control vector, known external influence
                          raw[1],                #TODO adjust for rotation already ajusted
                          raw[0],
                          raw[1]
                        ])
        ...
        x_0 = x1
        p_0 = p1
        print(np.ravel(x_0[0])[0], np.ravel(x_0[1])[0], x_0[2], x_0[3])
        sleep(0.01)
    E.stop()
    AccelBind.stop()
